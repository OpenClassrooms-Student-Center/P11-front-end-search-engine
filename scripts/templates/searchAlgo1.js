 ...
 */
export function search(request, appliance, ustensile, ingredients) {
  console.time("search");
  let recipes = data.recipes;
  recipes = matchAppliance(recipes, appliance.toLowerCase());
  recipes = matchUstensils(recipes, ustensile.toLowerCase());
  recipes = matchIngredients(recipes, ingredients);
  recipes = matchContent(recipes, request.toLowerCase());
  //save the result
  lastSearch = [appliance, ustensile, ingredients, recipes];
 ...
 * @param {array} recipes - array of objects
 * @param {string} ustensile
 * @returns {array} - array of objects (corresponding recipes)
 */
function matchUstensils(recipes, ustensile) {
  let recipesMatched = [];
  for (let recipe of recipes) {
    if (
      ustensile == "" ||
      recipe.ustensils.filter((usten) => usten.includes(ustensile)).length > 0
    ) {
      recipesMatched.push(recipe);
    }
  }
