...
// bench 1
const filteredRecipe = recipes.filter((result) => {
    const searchInputString = "poulet";
  if (
    result.name.toLowerCase().includes(searchInputString) ||
    result.description.toLowerCase().includes(searchInputString) ||
    result.ingredients.find((items) => {
      return items.ingredient.toLowerCase().includes(searchInputString);
    }) != undefined
  ) {
    return result;
  }
...
for (let recipe of recipes) {
   ...
   var searchInputString = "poulet";
     
  if (
    recipe.name.toLowerCase().includes(searchInputString) ||
    recipe.description.toLowerCase().includes(searchInputString)
  ) {
    recipesFiltered.push(recipe);
  }
  for (let ingredients of recipe.ingredients) {
    const ingredient = ingredients.ingredient.toLowerCase();
    if (ingredient.includes(searchInputString)) {
      recipesFiltered.push(recipe);
    }
  }
}
