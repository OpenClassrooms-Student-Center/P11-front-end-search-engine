...
/* ======================== END - DOM =====================  */ /* ======================== START - Algorithme =====================  */ const filterByName = (recipes, filtre)=>{
    recipes.filter((recipe)=>recipe.name.toLowerCase().includes(filtre.toLowerCase())
    );
};
const filterByIngredient = (recipes, filtre)=>{
    recipes.filter((recipe)=>recipe.ingredients.find((ingredient)=>ingredient.ingredient.toLowerCase().includes(filtre.toLowerCase())
        )
    );
};
...
function searchByName() {
    if (searchBar.value.length >= 3) {
        ...
        render(results);
    } else if (searchBar.value.length <= 2) renderRecipe(recipes);
}
...
//# sourceMappingURL=index.1ae7f3d1.js.map
